...
public class PartnerTimeLineAdapter extends BaseAdapter
{	
	Context				context;
	...
	public PartnerTimeLineAdapter(List<TimeLineModel> list, Context context, String className)
	{
...
//		else hohder.header.setVisibility(View.GONE);
		...
		if (tempLogModel.temprature != 0f && tempLogModel.temprature > 80) if (PeriodTrackerObjectLocator.getInstance().getTempUnit().equals(context.getResources().getString(R.string.celsius))) hohder.tempTextView.setText("Temp: "+(Utility.getStringFormatedNumber(String.valueOf((Utility
				.ConvertToCelsius(tempLogModel.temprature)))) + " " + PeriodTrackerObjectLocator.getInstance().getTempUnit()));
		...
		else if (PeriodTrackerObjectLocator.getInstance().getTempUnit().equals(context.getResources().getString(R.string.celsius))) hohder.tempTextView.setText("Temp: "+(Utility.getStringFormatedNumber(String.valueOf((Utility.ConvertToCelsius(Float.parseFloat(PeriodTrackerObjectLocator.getInstance()
				.getDefaultTemperatureVaule()))))) + " " + PeriodTrackerObjectLocator.getInstance().getTempUnit()));
...
//		hohder.empty_sym.setVisibility(View.GONE);//sazid
		...
		if (tempLogModel.id != -1)
		{
			...
			{
				hohder.listViewforMoods.setAdapter(new CalendarMoodandSymtomListAdaptor(interfaces,tempLogModel.moodSelectedsMap, PeriodTrackerConstants.MOOD_BASE_FRAGMENT, context));
...
//				hohder.empty_mood.setVisibility(View.VISIBLE);
			...
			if (null != interfaces && interfaces.size() > 0)
			{
				hohder.listViewforSymtoms.setAdapter(new CalendarMoodandSymtomListAdaptor(interfaces,tempLogModel.symtomsSelectedModelsMap, PeriodTrackerConstants.SYMTOM_BASE_FRAGMENT, context));
