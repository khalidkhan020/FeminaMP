...
public class App42GCMService extends GCMBaseIntentService
{
	...
	protected void onMessage(Context context, Intent intent)
	{
		...
		if (((APP) context.getApplicationContext()).getPreferences().getBoolean(APP.PREF.SHARE_NOTIFY.key, true))
		{
	...
	public void generateNotification(Context context, String message)
	{
		...
		catch (Exception e)
		{
			// TODO Auto-generated catch block
			e.printStackTrace();
			notificationIntent = new Intent(/*context, LoginScreen.class*/);
		...
		Notification notification = new NotificationCompat.Builder(context).setContentTitle(title).setContentText(message).setContentIntent(intent).setSmallIcon(icon).setWhen(when).setNumber(++msgCount).setLargeIcon(getBitmapFromURL(LARGE_IMAGE_URL)).setLights(Color.YELLOW, 1, 2)
